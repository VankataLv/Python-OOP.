...
class Registration:
    ...
    def add_user(self, user: User, library: Library) -> str:
        if user not in library.user_records:
            ...
            return f"User with id = {user.username} already registered in the library!"
    ...
    def change_username(self, user_id: int, new_username: str, library: Library) -> str:
        try:
            user_to_edit = next(filter(lambda u: u.user_id == user_id, library.user_records))
        ...
        try:
            library.rented_books[new_username] = library.rented_books.pop(user_to_edit.username)
        except KeyError:
            pass
